...
\caption{\sys allows multiple decorrelations to compose, and can reveal them in
    ...
    speaks-for records allows \sys to handle recorrelation of intermediate
...
%
For example, as shown in Figure~\ref{f:composition} (left-hand side), if Bea
...
(Figure~\ref{f:composition}). (1) The ``bears''
decorrelation rewrites Ewok posts to pseudoprincipal $P_1$; and (2) the ``Star
...
(3) If Bea first reveals the ``bears'' anonymization---the first applied
...
%
(4) Now, if Bea reveals the second applied disguise---``Star Wars''---\sys finds
