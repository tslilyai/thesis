...
\caption{\sys allows multiple decorrelations to compose, and can reveal them in
    ...
    applied decorrelations. \sys enables this via the speaks-for chain encoded in
    speaks-for records: when AnonPig is recorrelated by revealing ``Star Wars''
    (3), \sys removes the link and links the principal prior (Bea) to the
    principal after (AnonFox). Later, when Bea reqests reveal of their ``Bear''
    posts (4), \sys determines that the principal to recorrelate
    to---AnonPig---no longer exists, and finds the principal preceding AnonFox
    in the speaks-for chain,
    which is Bea.}
