...
the \xx operation.
%
\todo{Reveal credentials let \sys recursively decrypt speaks-for records, and
...
challenge.
%
\sys's semantics (\S\ref{s:hlsemantics}) require that data that is decorrelated
...
\sys rewrites the removed ``Ewok'' row in the diff record so that it
        references Bea instead of $P_1$,
