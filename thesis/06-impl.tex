...
\end{tabular}
    ...
    of the logic for reveal is embedded in the record implementation (\eg a
...
\sys stores the replay log as a vector of updates function pointers with
...
also introduces potential errors if an item needs to update its primary key as
well (\eg the disguise specification instructs \sys to disguise the user ID with
a pseudoprincipal ID, but the user ID column also acts as a primary key). If
\sys detects a change to a primary key, \sys individually updates rows'
primary keys prior to the batch update, ensuring that rows sent in the
