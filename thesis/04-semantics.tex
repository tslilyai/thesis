...
\todo{Although this example demonstrates composition of two decorrelation disguises on
Bea's data (\S\ref{s:semantics:hl:composition}), the same semantics would occur
had multiple disguises been applied to all users' data, including Bea's data.
This can happen, for example, if an
application admin decides to globally anonymize all posts. However, these disguises must still be revealed with one of Bea's
reveal credentials.}
