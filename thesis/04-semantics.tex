...
reason about the resulting state of data after a disguise and reveal, as well as 
an overview of \sys's API.
...
\subsection{\texttt{DisguiseData}}
    ...
    (\S\ref{s:semantics:hl:composition}).  Depending on the disguise
    specification, this will remove, modify, or create a new pseudoprincipal
    owner for data with a foreign key to a pseudoprincipal instead of the user.
    %
...
%
    ...
    or \texttt{RETAIN}) to ensure that reveals of decorrelations preserve
        referential integrity. 
            %
            \sys requires this argument because pseudoprincipals may acquire new references
            from application objects inserted after the time of disguise---for
            example, a decorrelated comment might have new responses. To ensure
            that revealing a decorrelate operation---which deletes
            pseudoprincipals---preserves referential integrity, developers 
            inform \sys how to, during reveal, handle these objects added after
            disguising that refer pseudoprincipals of the disguise.%
    ...
    \paragraph{Return Value.} 
    Returns true if \sys reveals all rows disguised by \texttt{did} successfully, and false otherwise.
