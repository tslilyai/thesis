...
// Registers u for disguising and returns u's backup credential
...
\subsection{\texttt{RegisterPrincipal}}
    ...
    \texttt{RegisterPrincipal} returns a backup token that the application
    should return to the user client as a backup reveal credential.
    ...
    credential (\S\ref{s:impl:cred}).\footnote{An alternative API could take
    only a user's password, and \sys
    would generate a fresh keypair for the user. In this case, \sys would return both the backup
    credential and the private key as possible reveal credentials.}  This allows the user to later provide any
    of their private key, their password, or their backup credential to unlock
    and reveal their disguised data. 
%
    \sys saves the provided public key in order to disguise and encrypt a user's
