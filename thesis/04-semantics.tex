This chapter describes informal high-level semantics that a developer can use to
...
%
Disguising already-disguised data only decreases the amount of information
...
(Figure~\ref{f:api-high}). This section
describes each API function and how developers would use them.

\vspace{-6pt}
\subsection{\texttt{RegisterPrincipal}}
    \texttt{RegisterPrincipal} registers an application user as a principal
    whose data can be disguised and revealed. Unique users should be registered
    only once. Once registered, an undeleted user's data can always be disguised
    and revealed. Users deleted by a disguise do not need to reregister if
    revealed.
    ...
    \begin{longtable}{|m{0.13\textwidth}|m{0.8\textwidth}|}
        ...
        \hline
        \texttt{pubkey} & (Optional) the user's public key (used to encrypt the
        user's disguised data).\\
        \hline
        \texttt{privkey} & (Optional) the user's private key (later usable as a reveal
        credential).\\
        \hline
    \end{longtable}
    \end{center}
    ...
    \sys uses the password and (generated or provided) private key to store a
    ...
    their private key, their password, or their backup credential to
    ...
    After registration, \sys forgets the user's private key and password, if
    provided.
        
\subsection{\texttt{DisguiseData}}
\label{s:semantics:dd}
    ...
    which the application should forward to disguised users in case they wish to later
...
%
%%%%% LYT TODO
However, \sys decorrelates any two rows from two different tables to 
two different pseudoprincipals, as well as any two rows referring to different
users. Two different disguises always produce disjoint sets of pseudoprincipals.
%no two objects referring to different users or from different
%database
%tables refer to the same pseudoprincipal, and no two disguises' sets of produced
%pseudoprincipals overlap.
%
...
rows).
%
For all other update specifications, \sys assumes that any nondeterminism or
side effects from performing the update will maintain application correctness.
    %
As a result, \sys's API cannot support some types of updates. For example,
consider a global update that calculates the current vote count of a story 
and adds this value as a column attribute to the story row.
%
    This update depends on vote rows present in the database.
%
    If a disguise has removed both a user's story and its votes, then upon
    reveal, \sys will maintain referential integrity by revealing disguised
    stories before disguised votes.
%
    But when applying the vote count update during reveal of the story, \sys
    will find 0 votes because the votes have not yet been restored.
%
...
%
If a disguise is user-specific, then \sys disguises only data that has a foreign key to that
user's identifier and matches the disguise
specification's predicate. Invoking a disguise with
\texttt{u\_opt = Some(uid)} thus adds a condition on a disjunction of
...
true} to disguise all data of a table turns into \texttt{WHERE true AND
...
(Developers should instead use decorrelation to remove principal rows
...
\begin{figure}
    ...
    \centering
    \includegraphics[width=.7\textwidth]{figs/composition-hl-inorder}
...
\begin{subfigure}[t]{.47\columnwidth}
    \centering
    \includegraphics[width=.7\textwidth]{figs/composition-hl-ooo}
...
\end{subfigure}
    \caption[\sys only reveals the original state of data disguised multiple
    times once all disguises applied to the data have been revealed.]{
        \small If Bea decorrelates an ``Ewok'' story twice, \sys only recorrelates
        the story back to Bea once Bea reveals \emph{both} disguises,
        regardless of the reveal order. (a) 
        illustrates an in-order reveal, where Bea reveals the most
        recent disguise first. (b) illustrates an out-of-order reveal, where
        the oldest disguise is revealed first.}
...
\begin{enumerate}
    ...
    \item[(3)] Bea next reveals one of their disguises:
        ...
        After reveal of the ``bears'' disguise, any unrevealed disguises remain
        applied to the ``Ewoks'' story.
        %
...
to a user's data. If Bea disguises their data multiple times, then the
application repeatedly invokes \texttt{DisguiseData} on behalf of Bea
(\texttt{u\_opt = Some("Bea")}).  This causes \sys to disguise only data
associated via a foreign key to Bea's application principal
(\S\ref{s:semantics:dd}).
%
However, this means that \sys will not disguise Bea's data that has already been
decorrelated to a pseudoprincipal by a prior disguise.
If Bea wants to disguise already-decorrelated data, \sys must have Bea's reveal credentials (\texttt{disguise\_over = Some(cred)}). This allows
\sys to access Bea's correlations to pseudoprincipals in order to disguise their
pseudoprincipals' data.
%
\sys will then treat all of Bea's pseudoprincipals' data as Bea's, and
remove, modify, or create a new pseudoprincipal owner for this data depending on
the disguise specification.
%
...
%
Disguise composition may also occur if, for example, an admin globally disguises
all users' data (\texttt{u\_opt = None}). In this case, \sys disguises all data satisfying the
disguise specification's predicates, regardless of which user owns the data, 
and thus does not need reveal credentials to
discover which pseudoprincipal users to disguise.
%however, a user with access to all data (\eg an application
...
to pseudoprincipals.
%
For example, any owner can reveal a shared message, which restores the message
to the database if it does not exist, and recorrelates only the revealing user;
all disguised owners remain decorrelated as pseudoprincipals until they also
choose to reveal the message.
%
...
Chris share a Lobsters message. \sys associates Bea with AnonPig and Chris
with AnonFox for this shared message.
...
it again.
%
%Extensions to \sys's design could allow \sys to
%reveal the data once the user reveals all subsequent disguises that modified or
%removed the data.
%
...
follows:
\begin{enumerate}[nosep]
    \item[(1)] Disguise $D_0$ of user $u$ modifies some data row.
    \item[(2)] A subsequent disguise $D_1$ removes the same data row. 
\item[(3)] If $u$ first reveals $D_0$, \sys cannot reveal the data because
    ...
    \item[(4)] When $u$ later reveals $D_1$, \sys decrypts $u$'s log, finds the
        ``reveal-$D_0$'' update, and applies the update to reveal $D_0$'s
        modification of the data prior to restoring it.
\end{enumerate}

\paragraph{API.} \sys's API assumes that:
\begin{enumerate}[nosep]
    \item[(1)] the application uses a relational database;
    \item[(2)] rows to \xx have direct foreign key relationships to a users table,
    where each user corresponds to a row of that table;
\item[(3)] all rows to \xx are owned by (have a foreign-key relationship) to one or more principals; and
\item[(4)] all rows can be uniquely identified (\eg via primary key).
\end{enumerate}
%
