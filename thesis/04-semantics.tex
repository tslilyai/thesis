...
\subsection{\texttt{DisguiseData}}
    ...
    \begin{longtable}{|m{0.18\textwidth}|m{0.75\textwidth}|}
        ...
        \hline
        \texttt{spec}& The disguise specification (\S\ref{s:semantics:spec})
...
\\
    ...
    \todo{discussion why API is this way, forward reference to spec.}
...
When a prior disguises have applied, users may 
wish to disguise data that has already been decorrelated 
(\S\ref{s:semantics:hl:composition}).
\sys allows this via an optional reveal credential provided in
\texttt{disguise\_over}. If \texttt{disguise\_over} is \texttt{Some(cred)}, \texttt{u\_opt} must be
