...
\subsection{\texttt{RegisterPrincipal}}
    ...
    After registration, \sys saves only the user's public key, and forgets the
    user's private key and password (if provided).
...
%
\sys will then treat all of Bea's pseudoprincipals' data as if it belongs
to Bea, and---depending on the disguise specification---will remove, modify, or
create a new pseudoprincipal owner for these pseudoprincipals' data.
%
...
to pseudoprincipals.
%
For example, any owner can reveal the message, which restores the message to the database if
