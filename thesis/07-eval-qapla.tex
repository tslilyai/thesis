...
\begin{figure}[t]
  \centering
    ...
    outperforms Qapla \newstuff{on nearly all common WebSubmit operations} \oldtext{on nearly all operations in WebSubmit} (2k users,
    80 answers/user).
...
undisguised) data.\oldtext{, and anonymizing
accounts. (lyt: note, one particular query during account anonymization for Qapla improved by an order of
magnitude with index optimizations that we added when rerunning experiments,
leading to comparable numbers to Edna for this operation.)}
%
