...
%
\todo{check} 
(3) Application contents constantly change due to developer
updates (\eg moderations) or schema migrations (\eg adding new columns or
tables).  In order for \sys to reveal disguised data without overwriting these
changes or invalidating structural invariants, developers write
\emph{application update executables} that encapsulate these changes applied to
a single user's data. Developers then invoke \sys's API to record this update
(and the time of the update), so \sys can apply changes as needed to user data
upon reveal.
%
...
\todo{xxx}
If the developer updates stories (\eg by normalizing any URLs), the developer
performs a bulk update in the Lobsters applications. The developer also writes
an update executable encoding how to update individual user's stories' URLs as
well. Then the developer invokes \sys's \texttt{record\_update} API call with
...
%
Oftentimes, these executables may be a subroutine of the bulk update that performs the
whole application change. 
%
