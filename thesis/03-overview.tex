...
initiated by an admin or developer, like schema changes or global updates (\eg normalizing URLs of all stories). 
%
However, if the developer updates the database after some data is disguised and
before \sys reveals it, then the update will fail to apply to the disguised data
(as it remains inaccessible to the application).
%
Instead, the developer must inform \sys about the update so \sys can apply the
update to the disguised data prior to reveal.

When a database update is performed, the developer writes a corresponding
reveal-time update specification that captures the effects of the operation and
invokes \sys's \texttt{record\_update} API call with the specification. \sys
stores these updates in a replay log, which lets \sys reveal disguised data
without overwriting the associated application changes.
%
...
instead of rows. In these scenarios, the developer must write a separate update
specification for updating disguised data rows.
%
For example, Lobsters has a schema migration that adds a column to the
\texttt{users} table.  The bulk operation simply performs an \texttt{ALTER
TABLE} query, while the update given to \sys receives user rows without the
column as input, and outputs user rows with the column.
%
...
attribute, and say that a disguise has removed both a user's story and its votes.
%
...
storys before revealing those for votes in order to maintain referential
integrity. But when applying the \texttt{vote\_count} update to story diff
records, \sys will find 0 votes for the story, because the votes have not yet
been restored.
%
\sys supports this update only if this is correct application behavior (scenario
\two{}).
%
