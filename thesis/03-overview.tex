...
invokes \sys's \texttt{record\_update} API call with the update
specfication. \sys
stores these update specifications in a replay log, which lets \sys reveal disguised data
...
\texttt{users} table.  The global update simply performs an \texttt{ALTER TABLE}
query, while the update speification given to \sys receives user rows without the column
...
then when \sys reveals the disguise, \sys will first reveal the diff records for
