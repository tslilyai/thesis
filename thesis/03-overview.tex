...
unavailable~\cite{dnefs,townsend:cryptoshredding,aws:cryptoshredding,gtr:cryptoshredding}.\footnote{For
usability, \sys accepts as credentials either a user's private key, or a user's
password from which \sys can derive the private key. Crypto shredding assumes
sufficiently random key material, so \sys here assumes the user's password to be
as strong as the private key.}
%
