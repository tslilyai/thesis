...
permanent account deletion \todo{more tools? account deactivation?} that result
...
orphaned data or break
referential integrity---for example, one row might have a foreign key to a
deleted row of another table, leaving a dangling reference---which
requires application changes to handle correctly. These solutions also lack
...
%
\lyt{Finally, application databases often undergo management operations, such as
schema migrations. These operations would fail to affect any disguised
data: it remains inaccessible to the application or developer. However, the
disguised data would need to reflect these global database changes in order to
preserve correctness upon reveal.}
%
