...
%
\lyt{Not really sure what this first part is for; should we just jump into
motivation?}
In this introduction, we motivate the need for data disguising for flexible
...
\begin{enumerate}[nosep]
    \item The identification and exploration of \emph{disguised data} for
        flexible user data privacy in web applications.

    \item Disguised data abstractions for developers and users of web
        applications, namely \xxing and revealing transformations; \emph{disguise
        specifications} written with a small set of data-anonymizing primitives
        (remove, modify, decorrelate) , which cover a wide range of application
        needs and compose cleanly; and \emph{reveal credentials} to allow users
        to reveal their data (Ch.~\ref{ch:overview}).

    \item Abstractions to implement disguised data, including pseudoprincipals,
        diff records, and speaks-for chains.
        (Ch.~\ref{ch:design})

    \item A design and realization of these abstractions in \sys, a prototype
