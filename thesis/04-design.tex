...
\sys supports these joint ownership semantics by modifying its design for
...
(*\textbf{ApplyUpdate}*)(update_spec: UpdateSpec);
\end{lstlisting}
...
Developers may find \sys's security model unnecessarily strong---perhaps they do
not worry about data breaches, care about regulations like the GDPR, or trust their application code to not expose
disguised data. Instead, these developers may want to know whether \sys can help
support disguised data without encryption.
%
This section discusses which aspects of \sys would become obsolete; which would
still help the developer support disguised data; and potential alternative
designs for disguised data without encryption. 

\paragraph{Unnecessary Aspects of \sys.}
If \sys removes encryption of disguised data, several aspects of \sys's design
become unnecessary.
%
For example, \sys would no longer need application of global database updates
during reveal, since disguised data remains accessible to the application, and
updates can thus also apply directly to disguised data.  However, developers may
still need to write data transformations corresponding to these updates in order
to update disguised data rows stored in new database tables.
%
...
they wish to disguise their disguised data again using a
boolean.

%
\paragraph{Useful Aspects of \sys.}
Other aspects of \sys can still help developers solve disguising and revealing
challenges unrelated to encryption.
%
Without encryption, \sys could provide developers with a new API
(Figure~\ref{f:newapi}) similar to the one described in \S\ref{s:api}, but with
disguised data no longer removed from the database via encryption. 
%
...
%
This API still helps address the challenges of maintaining referential integrity when
a disguise is performed (creating pseudoprincipals as placeholder users);
...
%
\paragraph{Alternatives to \sys Without Encryption.}
Instead of moving disguised data into a new database table for disguised data,
as \sys does, developers could also choose to implement disguised data by
logically removing it using database flags and predicates (\eg
\texttt{is\_deleted}) because disguised data need not be encrypted.
%
...
Qapla~\cite{qapla}---which this thesis compares to \sys in \S\ref{s:eval-qapla}---
