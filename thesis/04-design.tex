...
\sys's revealing as described can reveal data that ignores database changes applied
since the time of disguise, such as application updates to un\xxed data or
schema migrations.
%
\sys utilizes developer-provided information about important updates and schema
migrations to apply them to \xxed data prior to revealing it, ensuring that
revealed data reflects the current state of the database.
%
Prior to revealing the udpated data, \sys will also perform consistency checks
to guarantee adherence to database structural invariants, such as uniqueness
constraints.
%
...
%
However, \sys could correctly restore the post if it knew to remove the
swear words prior to reveal.
%
...
\emph{application updates replay log}.
%
...
log.
%
When revealing data, \sys would apply, in order, every entry since the time of
disguise in the replay log to the disguise's diff records.  
%

%
Log entries map the original and new database objects in diff records to
corresponding sets of updated-original and updated-new database objects. 
%
After applying all updates, \sys uses the updated database objects to remove the
updated-new placeholder data from the database and restore the updated-original
disguised data.
...
%
In addition, replay logs only apply to diff records, which contain the actual data
changes; \sys assumes that the identifiers for principals in speaks-for records
...
\subsubsection{Checking for Structural Invariants} 
...
to ensure that revealing will not violate the database's structural integrity.
%
