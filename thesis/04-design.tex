...
columns).
%
All types of diff records contain the \xx ID, but contain various original and
placeholder rows as follows:
\begin{itemize}[nosep]
...
%
Log entries map the original data and the placeholder data in diff records
...
update to placeholder data as well). Only then does \sys restore the post
with no swear words (and had placeholder data existed, \sys would remove the
...
\sys can utilize 
\todo{k9db strategy for owernship chains, entity relationship graph in FB, etc.}
...
assumptions hold; otherwise, \sys currently cannot correctly support this
update.
%
...
\todo{go and find other mentions of limitations within design section?}
