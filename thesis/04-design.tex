...
\begin{figure}
    ...
    words, it can invoke \sys to log the corresponding reveal-time update specification. \sys then applies the
...
\begin{figure}
    ...
    \caption[During reveal, \sys applies reveal-time update specifications for
    schema migration updates.]{Similar to a moderations update, the
    application can notify \sys of a schema migration to apply to diff records
    prior to reveal. Yellow highlights changes to the application data or \sys's
    data. For simplicity, only the placeholder data of a pseudoprincipal row is
    shown; in reality, placeholder data exists as part of \eg a decorrelation
    diff record.}
...
\begin{figure}
    ...
    pseudoprincipals and diff records.]{\sys implements joint ownership semantics by storing a fully
...
Figure~\ref{f:shared:data} depicts an example execution of \sys over shared
data:
%
\begin{enumerate}
...
normal. 
        \todo{\sys also restores the partially-removed record to store the
        completely decorrelated remove diff record.}
%
...
One potential solution if foreign keys to users act as unique row identifiers
...
Second, owners of shared data who remove and reveal the data multiple times will
decorrelate to the same pseudoprincipal each time.
\todo{elaborate here.}
