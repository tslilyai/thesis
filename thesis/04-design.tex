...
%
When another owner's disguise removes the data (2), \sys stores a copy of the
...
%
As shown in (2), if the owner also happens to be the last remaining owner (all
other owners are pseudoprincipals, \ie all other owners have removed the data),
\sys removes the shared data row from the application database, and the
fully-decorrelated diff record from \sys's partially-removed table.
%

%
Now, when \emph{any} owner chooses to return (3), \sys first restores the
...
normal.
%
If a subsequent owner reveals the shared data (4), \sys's consistency checks will
...
the database (\eg aggregates over the number of users). They may also affect
data that the application reads or has read to send notifications or perform
other side affects.  \sys currently expects the developer to correctly handle
these scenarios, and ensure that any aggregate results or placeholder data does
not violate application correctness.
%
...
%
\todo{should this be here or part of spec in \S3?}
