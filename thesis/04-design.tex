...
migration can invoke \sys with an update executable encoding this schema
...
%
The migration update executable would map an original \texttt{users} data row
...
\texttt{users}, as shown in step (3). These rows would be restored to the
database (4).
%
The executable would also generate two rows for any placeholder \texttt{users} data row in a
diff record and remove these from the database (4).
%
