...
operations requires 81 LoC; this adds to the 89 LoC that the developer already
writes to implement the bulk database management operations themselves.
%
...
latencies for revealing operations supported only by \sys
...
\sys adds both space and compute overheads to the application; this section
measures the
...
%
\sys's record storage uses 12 MB after the initial creation of the five tables
holding \sys's data (the disguise table, principals table, the deleted principal
table, the user-shares table, and the replay log table); this includes the space
for indexes and other InnoDB metadata.
%
...
\begin{figure}
    ...
    writing separate update functions; however, management operations like
...
\texttt{show\_email} and add \texttt{story\_texts} as bulk table updates, and
thus write separate updates that operate on individual rows, taking 54 and 27
LoC respectively.
%
...
\begin{figure}
    ...
    \begin{tabular}{m{0.15\columnwidth}|m{0.05\columnwidth}|m{0.22\columnwidth}|m{0.43\columnwidth}}
        \centering\textbf{Bulk Op Type} & \centering\textbf{\%} &
        ...
        url= NORMALIZE(url)} \\
        \hline
            Denormalized & \begin{center}10\end{center} & Update cached story vote count & \texttt{UPDATE stories SET
...
state of other database tables. In the investigated Lobsters database management
operations, two operations out of the 20 were unsupported, and both updated the
...
\begin{figure}
    ...
    \end{tabular}
    \caption{Applying the various updates adds greater overheads per story than
...
This section looks at how bulk operations and updates affect application
performance, and how updates affect \sys's reveal performance.
...
First, as a baseline, this section looks at the cost of performing bulk operations, which
applications incur without \sys (Table~\ref{tab:updates-perf}, line 1).
%

%
The URL normalization application update takes 5.63s, the longest of all
updates.
%
This matches the optimized bulk operation execution time of 6s in the deployed
...
reveal. \sys applies updates to data to reveal in batch, but the cost of
...
rows is more expensive than restoring the original rows, the application's bulk
operation would have incurred the cost of updating these rows had the rows not
been disguised at the time. Thus, \sys moves the cost of updating these rows to
reveal time, rather than bulk operation time (although the cost may be higher
because the update is not batched over all table rows).}
%
Updates do not affect the cost of restoring rows for all tables other than
