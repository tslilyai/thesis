...
and revealing operations take longer in \sys (12.6--40.6ms), but allow users to
...
%
\sys takes 59.2--71.5ms to \xx and reveal a PC member's data, again owing to the
...
%
With 80 PC members, this transformation takes 5.7s, which is acceptable for a
one-off operation.
%
...
%
The median latencies for entire-account removal or decay are small (8.6--12.1ms
for \sys, and 4.0--5.2ms for the baseline), since the median Lobsters user has
little data. Revealing \xxed accounts takes 15.3--22.0ms in the median.
%
Highly active users with lots of data raise the 95\textsuperscript{th}
percentile latency to $\approx$175ms for both \xxing for revealing.
%
Topic anonymization touches less data and is faster than whole-account
transformations, taking 3.3ms and 12.1ms for the median user to respectively \xx
and reveal.
%
...
pseudoprincipal (18.3ms and 19.0ms respectively); 50\% of this increased cost
...
%
HotCRP removal latencies also increase by $\approx$1ms per
pseudoprincipal (181.8ms) and restoration latencies increase by $\approx$2ms per pseudoprincipal
(365.6.4ms). Again, cryptographic operations add $\approx$0.5ms per
...
transactions) and up to $23$\% (with transactions at high load).
...
removal and revealing take 2.8 and 17.8 seconds under high load, and 1.8 and
10.0 seconds under low load. \lyt{Note: these numbers are about the same (if not
higher) when not using transactions.}
%
Revealing takes significantly longer than disguising due to the many more
queries required to perform consistency checks.  \todo{Look into the cost of
INSERT-UPDATE queries for reveal.}
%
This is acceptable: 50\% of data deletions at Facebook take five minutes or
longer to complete~\cite{delf}.
%

%high load disguise 2593 2694 2952
%high load restore 15947 16082 19043
%high load disguise txn 2811 2897 3031
%high load restore txn 17393 17822 18355
%expensive disguise 1759 1835 1984
%expensive restore 9972 10231 10551
%exp txn delete 1691 1811 1942
%exp txn restore 9852 10017 10199
%
%low load
%-0.03667745415
%-0.07011866235
...
\begin{comment}
high load disguise 4510 4711 5107
high load restore 11579 11727 11852
high load disguise txn 4477 4743 5034
high load restore txn 10403 10605 10837
expensive disguise 3001 3253 3560
expensive restore 6998 7219 7474
exp txn delete 2976 3218 3407
exp txn restore 6605 6895 7223
\end{comment}
