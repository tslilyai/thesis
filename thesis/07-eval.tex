\todo{Get numbers from Google Cloud}
...
is invoked by the application. Developers today already need to write these
application updates (\eg Lobsters implements these updates as Ruby Active Record
Migrations~\cite{ruby_arm}); our Rust/SQL implementation of the three updates
requires 187 LoC to implement the actual updates.
...
application update takes \todo{xxx}ms, the longest of all updates.
%
...
from the \texttt{stories} table takes \todo{xxx}ms.
%
Invoking \sys's hook to insert a new application update in \sys's replay log
takes \todo{xxx}ms, a negligible amount compared to the time to perform the
update/migration.
%
...
\texttt{story\_texts} both affect \texttt{stories}, and take \todo{xxx}ms and
\todo{xxx}ms respectively when applied to an individual \texttt{stories} diff
record.
%
...
%
Adding \texttt{show\_email} to \texttt{users} takes \todo{xxx}ms per user, but
...
a user wants to reveal; the cost to revealing an average user in Lobsters with \todo{xxx}
stories and \todo{xxx} comments increases by \todo{xxx}ms total, while the cost
to reveal the most expensive user with the most data (\todo{xxx} stories and
\todo{xxx} comments) increases by \todo{xxx}ms.
