Our evaluation seeks to answer \newstuff{five} questions:
%
\begin{enumerate}[nosep]
 %
 \item How much developer effort and application modification does \sys require? (\S\ref{s:eval-effort})
 %
%\item Can a policy-enforcement system like Qapla~\cite{qapla} be used to realize
   %\sys-like functionality? (\S\ref{s:eval:qapla}) %(Qapla) One WS graph
%
\item How expensive are common application operations, as well as
 ...
 %
%Add common operation to Lobsters/HotCRP (Common Read Operation / label explicitly, list papers, frontpage)
%3-way graph
%composition
%
\item What overheads does \sys impose, and where do they come from?
    (\S\ref{s:eval-overheads})
%Drill down (storage, impact on concurrent app throughput)
%Lobsters throughput graph
...
%akin to what developers might do today
...
%The added effort to use \sys lies in writing \xx specifications.
...
%
%Given \xx specifications, \sys
%
%\lyt{
    %\sout{
%In comparison, a manual approach must implement (via \eg templated SQL
%statements) \xxing and revealing database changes that correctly compose, as
%well as secure ways of storing \xxed data so only authorized users can reveal
%it.
%}}
%%
...
\sys in mind.
%\sys also allows developers to add and reason about their
%transformations in isolation, and enables credential-based revealing
%of the specified \xxed transformations, and composition with prior or
%future transformations.

%These three applications were not written with \sys in mind, yet \sys still
%allows developers to add and reason about their transformations in isolation.
%\lyt{XXX}
%These applications satisfy \sys's assumptions (\S\ref{s:impl}) with no
%modifications.
%
%

%And, most importantly, required no debugging.
%
%
%
%Doing this manually would be much more laborious: it would require writing templated
%SQL statements, that are condiitonal not only on the subject and are different
%for every transformation, but also take into consideration other previous transformations,
%as well as writing similar statements for revealing.

%specifications follow systematic patterns, and require few lines of code compared to application code.
...
%\newstuff{11$\times$ more references by other schema tables (which \sys queries
%to check referential integrity during reveal)}
...
%
\oldtext{Under both low and high load, the throughput of normal operations is
only slightly affected by concurrent \xxing, dropping by at most 5.8\% without
transactions (and $<17$\% with transactions).}
