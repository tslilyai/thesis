...
\begin{figure}
    ...
    \begin{tabular}{m{0.28\columnwidth}|m{0.2\columnwidth}|m{0.2\columnwidth}|m{0.2\columnwidth}}
        ...
        \hline
        Perform Update (1 story row) & 0.22ms & 0.01ms & 0.01ms \\
        \hline
        Perform Update (1 user row) & 0.20ms & 0.01ms & 0.01ms \\
    \end{tabular}
    \caption{Stats for updates.}
    \label{tab:updates}
\end{figure}
...
Rust instead of Ruby Active Record Migration, as shown in
Table~\ref{tab:updates}. We reuse the bulk URL normalization operation for the
update to \sys. We implemented the other two schema migrations as bulk table
updates, however, and thus wrote separate updates that operate on individual
rows.
%
As shown in Table~\ref{tab:updates}, the additional lines of code to write the
updates on top of the bulk operation are either 0, when the update acts as a
subroutine of the bulk operation, or less than 100 LoC.
%
