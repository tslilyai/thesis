...
\begin{enumerate}
%
    \item[(1)] queries the database to fetch comments and votes by Bea
affiliated with ``Star Wars'';
%
\item[(2)] creates a pseudoprincipal (\eg
``AnonFox'') for every ``Star Wars''-tagged story that Bea commented
on, and inserts it as a new user;
% into the database;
%
\item[(3)] modifies the database by rewriting comment
foreign keys to point to the created pseudoprincipals, and
removing Bea's votes on those stories;
%
\item[(4)] creates new speaks-for records that map Bea to the created
...
%
\item[(5)] encrypts the speaks-for and diff records with Bea's public key and stores
them; and
%
\item[(6)] returns a unique \xx ID to the application.
\end{enumerate}
...
%
At reveal time, \sys can efficiently find a given user's \xxed data 
by using decrypted \xx table indexes in the principal table.
%
...
principal's ID and the principal's public key when a disguise removes a
principal.  This lets \sys to restore the principal to \sys's principal table if
a user later reveals the disguise.
%
...
\xxed data that was removed from tables in topologically-sorted order
(constructing the dependency graph as in \S\ref{s:applying}).
...
corresponding disguise records.
%
...
%
\sys uses the password to reconstruct Bea's private key and retrieve and decrypt
Bea's records. \sys filters those records to find those with the requested \xx ID.
%
...
\vspace{-3pt}
    \item[(2)] The developer invokes \texttt{RecordUpdate} with a reveal-time
        update specification when performing a global update that must hold over
        disguised data when it is revealed (\eg moderations). \sys records the
        update specification in its replay log. 
\vspace{-3pt}
    \item[(3)] When revealing data, \sys applies, in order, every update
        specification in the replay log added since the time of disguise to the
        disguise's diff records. Update specifications map the original data and
        the placeholder data in diff records to corresponding updated-original
        and updated-placeholder data. In this case, the update specification
        moderates explicit story text.
\vspace{-3pt}
...
\begin{figure}
    \centering
    \includegraphics[width=.78\textwidth]{figs/update-migration}
    \caption[During reveal, \sys applies reveal-time update specifications for
    schema changes.]{\small The
...
%
However, the technique for global 
changes (\S\ref{s:design:dataupdates}) allows \sys to handle schema changes as well. 
%
...
The developer then chooses to allow users multiple addresses via a schema
change. To do so, they create a new \texttt{addrs} table, with a
foreign key to the users table, and populate \texttt{addrs} using the address
data in \texttt{users}.  Finally, they remove the \texttt{addr} column
from \texttt{users}.
%
Figure~\ref{f:update:mod} illustrates how \sys applies such a schema change 
to disguised data upon reveal.
...
\sys supports joint ownership semantics for shared data with a modified removal
design.
%
...
%
%(\eg when some users are gone but others
%remain, so pseudoprincipals replace the removed users). 
%
...
\begin{figure}
    \centering
    \includegraphics[width=.95\textwidth]{figs/shared_data}
    \vspace{-6pt}
    \caption[\sys implements disguising of shared data using
    pseudoprincipals and diff records.]{\small (1) When Bea removes their message, \sys
    creates and stores a fully-decorrelated diff record of the message
    (modifications are depicted using
...
\sys's approach for shared data has two main limitations. First, the
application cannot use foreign keys to its users table as unique identifiers for
...
%
%This issue is specific to removal of shared data.
%
...
might introduce a layer of indirection via a new unique identifier (\eg an
autoincrementing primary key). This could potentially add performance
overheads.

Second, owners of shared data who remove and reveal multiple times decorrelate
to the same pseudoprincipal each time. This may allow observers of the
application to determine which pseudoprincipal corresponds to which user
(although this falls outside of \sys's threat model).  To see why \sys requires
this limitation, imagine the following counterexample:
\begin{enumerate}
    ...
    \item[(2)] Chris reveals and then removes the message again, 
        leaving no data in the database. Chris now has a diff record of the
        message with Chris mapped to a \emph{new} pseudoprincipal $P_3$.
...
\end{enumerate}
%
The failure of Chris' reveal arises because Chris and Bea disagree on what
data---which pseudoprincipal references---to insert into the
database, should the shared data object no longer exist. Chris expects it to be
$P_3$, but Bea will restore to $P_2$ for Chris. 
...
%
Reveal-time updates in replay logs only apply to diff records, which contain the
actual data changes, and not to speaks-for records. \sys assumes that the
identifiers for principals in speaks-for records that encode the speaks-for
chain remain consistent as the database changes, allowing \sys to use its
composition techniques with speaks-for chains (\S\ref{s:design:oooreveals}) to
reveal multiple disguises in any order.
%
...
%dangling pointers. 
%
A perhaps more realistic but limited solution might require developers to
schedule periodic updates to fix any rows that have been revealed since the
update. However, this only works for an idempotent update, as it should not
incorrectly update already-updated rows again.  
%
...
%
\sys's design achieves its key goal of keeping disguised data confidential
between the time of disguising and revealing.
%
Some aspects of \sys's design help make \sys practical and deployable without
major application modifications. In exchange for greater usability, however,
\sys gives up stronger security.
%
...
%
Thus, an attacker who compromises the database could learn:
\begin{enumerate}[nosep]
  ...
  \item the pseudoprincipals currently registered, from \sys's
    principal table and the application database;
...
owning posts in both ``MIT'18'' and ``BayArea'' topics), un\xxed data (\eg
comments signed with the user's name), and \sys metadata (\eg that some
anonymous user has more \xxed data than another) to infer the identity of the
original owning principal.\footnote{For efficiency, \sys
stores \xxed data without breaking it into chunks or adding padding.}
...
guarantees security equivalent to that of the user's password.
...
does not require encryption (\S\ref{s:semantics:noencrypt}),
\sys still helps developers maintain referential integrity when disguising
(creating pseudoprincipals as placeholder users), compose disguises and reveals
in arbitrary orders, disguise shared data, and apply global updates to disguised
data.
%
...
If \sys does not encrypt disguised data, it remains accessible to
...
keypair, user reveal credentials, encryption and decryption on the disguise and reveal
...
they wish to disguise their disguised data again (\eg to remove their
already-anonymized posts) using a boolean.
%
...
%
In this case, the developer must ensure that all queries for the data include a predicate
on the tag, so that application does not return disguised data. Systems like
