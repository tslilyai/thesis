\documentclass[11pt,twoside,final,natbib=false]{mitthesis}

\input{preamble}

\begin{document}

%\frontmatter

% always arabic page numbering (default is roman in \frontmatter)
\pagenumbering{arabic}

\input{frontmatter/title}
\maketitle

\begin{abstract}
\input{frontmatter/abstract}
\end{abstract}
\cleardoublepage

\section*{Acknowledgments}
%\begin{spacing}{1}
  \input{00-acknowledgments.tex}
%\end{spacing}
\cleardoublepage

%\section*{Prior Publication}
%Parts of this thesis were previously published in two conference
%papers~\cite{chajed:gojournal, chajed:daisy-nfs}.
%\cleardoublepage

\tableofcontents
\clearpage
\listoffigures
\clearpage

% TODO list while writing \listoftodos

%\mainmatter

\chapter{Introduction}%
\label{ch:introduction}
\input{01-introduction.tex}

\chapter{Related Work}%
\label{ch:related}
\input{02-related-work.tex}

\chapter{\sys Overview}%
\label{ch:overview}
\input{03-overview.tex}

\chapter{Disguising and Revealing Semantics}%
\label{ch:api}
\input{04-semantics.tex}

\chapter{\sys's Design}%
\label{ch:design}
\input{05-design.tex}

\chapter{Implementation}%
\label{ch:impl}
\input{06-impl.tex}

\chapter{Case Studies}%
\label{ch:case_studies}
\input{07-casestudies.tex}

\chapter{Evaluation}%
\label{ch:eval}
\input{08-eval.tex}

\chapter{Conclusion}%
\label{ch:conclusion}
\input{99-conclusion.tex}

%\backmatter

\printbibliography

% single spacing for bibliography
%\begin{spacing}{1}
%\bibliography{n-str,paper,n,n-conf}{}
% also consider alpha (more informative citation keys)
%\bibliographystyle{plainnat}
%\end{spacing}

\end{document}
