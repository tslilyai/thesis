...
\item 
    ...
    functionality in a related system (Qap\-la~\cite{qapla}),
...
database queries and updates. Restoration requires additional
per-pseudo\-principal
...
\begin{figure}
    \centering
    \begin{tabular}{m{0.22\columnwidth}|m{0.03\columnwidth}|m{0.22\columnwidth}|m{0.42\columnwidth}}
...
\begin{enumerate}[nosep]
%
    \item a data transformation such as URL
normalization, which can be reused as an update spec to \sys; 
%
\item a schema change such as
generating tables from other tables (\eg \texttt{story\_texts} from
\texttt{stories}) or altering columns, which requires writing a new update
        spec for \sys; or
%
...
%
Users with little data (\eg fewer than five stories) experience no visible increase
...
took 1789s~\cite{lobsters:migration}.}
%
